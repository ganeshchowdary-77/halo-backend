...
public class IamDashboardController {
    ...
    public ResponseEntity<HaloApiResponse<Map<String, Object>>> getSystemMetrics() {
        ...
        // Application metrics
        Map<String, Long> applicationMetrics = new HashMap<>();
        applicationMetrics.put("underReview", applicationRepository.countByStatus(PolicyStatus.UNDER_REVIEW));
        applicationMetrics.put("approved", applicationRepository.countByStatus(PolicyStatus.PENDING_PAYMENT));
        applicationMetrics.put("rejected", applicationRepository.countByStatus(PolicyStatus.APPLICATION_REJECTED));
    ...
    public ResponseEntity<HaloApiResponse<Map<String, Object>>> getSystemAlerts() {
        ...
        if (underReview > 0) {
            alerts.put("applicationsUnderReview", underReview);
        }
